%student(Id,Name([Last,Fist]),grade([a1,a2,a3,test])).
...
%student(1002,_,grades([_,_,_Test])).
...
%student(_name(Name),grades(L)),member(100,L).
...
cuenta_elementos([],0).
cuenta_elementos([_|L],N):-cuenta_elemenntos(L,Tam),N is Tam+1.
